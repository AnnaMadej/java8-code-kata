...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream.flatMap(cus->cus.getWantToBuy().stream())
                                                     .map(Item::getName)
                                                     .filter(str->!itemListOnSale.contains(str))
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = customer ->
            customer.getBudget() >= customer.getWantToBuy().stream()
                                   .mapToInt(itemToBuy -> onSale.stream().filter(itemOnSale-> itemOnSale.getName().equals(itemToBuy.getName())).findFirst().map(Item::getPrice).orElse(0))
                                   .sum();
